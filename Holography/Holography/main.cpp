...
fftw_complex* fn = (fftw_complex*)malloc(sizeof(fftw_complex) * 2048 * 2048);
fftw_complex* Q = (fftw_complex*)malloc(sizeof(fftw_complex) * 2048 * 2048);
fftw_complex* t_image = (fftw_complex*)malloc(sizeof(fftw_complex) * 2048 * 2048);
fftw_complex* B = (fftw_complex*)malloc(sizeof(fftw_complex) * 2048);

//Variables for fft 
fftw_complex* P = (fftw_complex*)malloc(sizeof(fftw_complex) * 5000);
fftw_complex* w = (fftw_complex*)malloc(sizeof(fftw_complex) * 5000);
int N;
...
void Transform_image()
{
	...
	for (i = 0; i < H_ROW; i++)
	{
		for (j = 0; j < H_COL; j++)
		{
			P[j][0] = t_image[i][j].real;
			P[j][1] = t_image[i][j].img;
		}
		FFT();
		for (j = 0; j < H_COL; j++)
		{
